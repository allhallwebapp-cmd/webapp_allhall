document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function userCanReview() {
        ...
        ordersSnap.forEach(order => {
            if (order.val().items.some(item => item.id === id)) {
                hasPurchased = true;
            }
        });
        ...
        reviewsSnap.forEach(review => {
            if (review.val().userId === currentUser.uid) {
                hasReviewed = true;
            }
        });
    ...
    async function loadReviews() {
      ...
      ratingsSummaryEl.innerHTML = `
        <span class="text-3xl font-bold">${avgRating}</span>
        <div class="flex">${renderStars(parseFloat(avgRating))}</div>
        <span class="opacity-70">(${totalRatings} reviews)</span>
      `;
        ...
        reviewsListEl.innerHTML = reviews.map(r => `
          <div class="glass p-4 rounded-2xl">
            <div class="flex items-center gap-3 mb-2">
              <img src="${r.userAvatar || 'assets/profile-placeholder.png'}" class="h-10 w-10 rounded-full object-cover" alt="User Avatar"/>
              <div>
                <div class="font-semibold">${r.userName || 'Anonymous'}</div>
                <div class="flex text-sm">${renderStars(r.rating)}</div>
              </div>
            </div>
            <p class="opacity-80">${r.comment || ''}</p>
            ${r.image ? `<img src="${r.image}" onclick="showReviewImage('${r.image}')" class="mt-3 rounded-xl w-32 h-32 object-cover cursor-pointer hover:scale-105 transition-transform" alt="Review Image"/>` : ''}
          </div>
        `).join('');
      }
    }
    ...
    window.showReviewImage = (imgSrc) => {
        const modalHtml = `
            <div id="reviewImageModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onclick="this.remove()">
                <div class="relative w-full max-w-4xl max-h-full overflow-hidden" onclick="event.stopPropagation()">
                    <button onclick="document.getElementById('reviewImageModal').remove()" class="absolute top-4 right-4 text-white text-3xl font-bold z-10 bg-black/50 rounded-full w-10 h-10 flex items-center justify-center">&times;</button>
                    <img src="${imgSrc}" class="w-full h-full object-contain rounded-xl"/>
                </div>
            </div>
        `;
    ...
    function renderStars(rating) {
        const totalStars = 5;
        let starsHtml = '';
        const starStyle = `text-yellow-400 text-shadow-[0_0_8px_rgba(252,211,77,0.7)]`;

        for (let i = 1; i <= totalStars; i++) {
            if (i <= rating) {
                starsHtml += `<span class="${starStyle}">★</span>`;
            } else if (i - 0.5 === rating) {
                starsHtml += `<span class="${starStyle}">★</span>`;
            } else {
                starsHtml += `<span class="text-gray-500">★</span>`;
            }
        }
        return starsHtml;
    }
    ...
    async function renderReviewForm() {
      ...
      const canReview = await userCanReview();
      if (!canReview) {
      ...
      reviewFormContainerEl.innerHTML = `
        ...
        <form id="reviewForm" class="space-y-3">
          <div class="flex items-center gap-2" id="starRating">
            <label class="opacity-80">Your Rating:</label>
            <div class="rating-stars flex flex-row-reverse">
              ${[5,4,3,2,1].map(n=>`<label for="star${n}" class="star-label text-yellow-400 text-3xl cursor-pointer">
                <input type="radio" id="star${n}" name="rating" value="${n}" class="hidden" required>★</label>`).join('')}
            </div>
          </div>
          <textarea id="reviewComment" class="input w-full" rows="3" placeholder="Write your review here..."></textarea>
          <div>
            <label class="opacity-80 text-sm">Upload an image (optional)</label>
            <input type="file" id="reviewImage" class="input mt-1">
          </div>
          <button type="submit" class="btn-glow px-4 py-2 rounded-xl">Submit Review</button>
        </form>
      `;
      ...
      document.getElementById('reviewForm').addEventListener('submit', async (e) => {
        ...
        await db.ref('reviews/' + id).push(reviewData);
    ...
    if (urlParams.get('review') === 'true') {
        const reviewSection = document.getElementById('reviewFormContainer');
        if (reviewSection) {
